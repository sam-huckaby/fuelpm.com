    ...
    return (
        <>
            <AlertDialog.Root open={alertOpen}>
                ...
                <AlertDialog.Content className="fixed z-50 top-[50%] left-[50%] -translate-y-[50%] -translate-x-[50%] rounded p-4 bg-white w-[90vw] max-w-[500px] shadow-lg ">
                    ...
                    <div className="flex flex-row justify-end mt-2">
                        <AlertDialog.Cancel asChild>
                            <button className="text-stone-700" onClick={() => setAlertOpen(false)}>{ confirmDetails.cancel || 'Cancel' }</button>
                        </AlertDialog.Cancel>
                        <AlertDialog.Action asChild>
                            <button className={((confirmDetails.danger)? 'text-red-800': '') + ` font-bold ml-2`} onClick={confirmDetails.action}>{ confirmDetails.proceed || 'Proceed' }</button>
                        </AlertDialog.Action>
                    </div>
                </AlertDialog.Content>
            </AlertDialog.Root>
                ...
                <DropdownMenu.Content className="right-0 py-2 mt-2 bg-white rounded-md shadow-2xl w-44 border border-solid border-stone-700">
                    {
                        ...
                        props.items.map((cur, index) => 
                            (cur.separator)?
                                ...
                                <DropdownMenu.Item key={index} onClick={() => { (cur.confirm)? useConfirm(cur) : ((cur.onClick)? cur.onClick() : () => {}) }} className={`px-4 py-2 cursor-pointer hover:bg-stone-200 ` + cur.classes}>
                                    {cur.label}
                                </DropdownMenu.Item>
                        )
                    }
